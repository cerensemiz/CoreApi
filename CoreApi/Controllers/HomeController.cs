    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult AddStudent(StudentVM model)
        {
            try
            {
                ...
                {
                    return Problem("Bilgilerinizi eksiksiz girdiğinize emin olun! Tekrar deneyiniz!");
                }
        ...
        public IActionResult FindStudentsByYear(int? year)
        {
            try
            {
                if (year == null)
                {
                    ...
                    return Problem("Yıl değerini vermediniz!");
                }
                var result = _manager.GetAll(x => x.BirthDate != null && x.BirthDate.Value.Year == year).Data;
                if (result.Count() > 0)
                {
                    return Ok(result);
                }
